...
The document is organized as follows. We devote section $2$ to finite groups of Lie type and Deligne--Lusztig theory, with a focus on the construction of Lusztig's nonabelian Fourier transform. In section $3$ we discuss representation theoretic aspects of $p$-adic groups and their relation to finite groups of Lie type. We recall the definition of unipotent representations and construct the dual nonabelian Fourier transform. Section $4$ is the most technical; there we discuss in detail how to explicitly compute the restriction functors $\res_{\un}^{\cpt}$, a method developed by Reeder in \cite{Reeder2000}. The remaining sections contain explicit computations for exceptional groups, and we collect these in large tables in the end of the paper.
